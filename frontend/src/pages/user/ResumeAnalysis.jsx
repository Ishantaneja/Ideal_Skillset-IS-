  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Resume Management & Parsing</h1>
      ...
      {uploading ? (
        <Card className="bg-brand-50/50 border-dashed border-2 border-brand-300 p-8 text-center">
          <div className="max-w-md mx-auto space-y-3">
            <div className="w-12 h-12 rounded-full bg-brand-100 text-brand-600 flex items-center justify-center mx-auto animate-pulse">
              <UploadCloud className="w-6 h-6" />
            </div>
            <h3 className="text-sm font-bold text-slate-900">{uploadStage}</h3>
            <ProgressBar value={uploadProgress} color="brand" showLabel={false} />
            <p className="text-[11px] text-slate-500">Please wait while the file is uploaded, scanned, and parsed.</p>
          </div>
        </Card>
      ) : (
        <div
          ...
          className="border-2 border-dashed border-slate-300 hover:border-brand-500 bg-white hover:bg-slate-50/50 rounded-2xl p-6 text-center cursor-pointer transition-colors group shadow-xs"
        >
          <div className="flex flex-col items-center justify-center space-y-2">
            <div className="w-12 h-12 rounded-xl bg-slate-100 group-hover:bg-brand-50 text-slate-500 group-hover:text-brand-600 flex items-center justify-center transition-colors">
              <UploadCloud className="w-6 h-6" />
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-800">
                <span className="text-brand-600 underline">Click to upload</span> or drag & drop resume
              </p>
              <p className="text-xs text-slate-400 mt-0.5">PDF or DOCX • Maximum file size 10 MB</p>
            </div>
          </div>
        </div>
      )}
            ...
            <Card title="Active Resume Document">
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-start space-x-3">
                  <div className="p-2.5 bg-red-50 text-red-600 rounded-lg border border-red-200 shrink-0">
                    <FileText className="w-6 h-6" />
                  </div>
                  <div className="overflow-hidden flex-1">
                    <div className="flex items-center space-x-1.5">
                      <h4 className="text-xs font-bold text-slate-900 truncate" title={activeResume.original_filename}>
                ...
                <div className="pt-2 flex items-center justify-between border-t border-slate-200/80 text-xs">
                  <span className="text-slate-500 flex items-center">
                    ...
                    Status: <span className="font-semibold text-slate-700 ml-1 capitalize">{activeResume.parsing_status}</span>
                  </span>
                  <button
                    onClick={() => setShowRawText(!showRawText)}
                    className="text-brand-600 hover:text-brand-700 font-semibold flex items-center text-[11px]"
                  >
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {resumes.map((item) => (
                    ...
                    >
                      ...
                      >
                        <p className="font-semibold text-slate-900 truncate">{item.original_filename}</p>
            ...
            {selectedSection === 'skills' && (
              ...
              >
                {parsed.skills?.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No recognized technical skills found in resume.</p>
                ) : (
                  <div className="space-y-4">
                    {Object.entries(skillsByCategory).map(([category, skillList]) => (
                      <div key={category} className="space-y-1.5">
                        <h4 className="text-xs font-bold text-slate-600 uppercase tracking-wider">
                          {category} <span className="text-slate-400">({skillList.length})</span>
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {skillList.map((skillName, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center text-xs bg-slate-100 text-slate-800 border border-slate-200 px-3 py-1 rounded-lg font-medium shadow-2xs"
                            >
            ...
            {selectedSection === 'education' && (
              ...
              >
                {parsed.education?.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No academic degrees parsed from document.</p>
                ) : (
                  <div className="space-y-3">
                    {parsed.education.map((edu, idx) => (
                      <div key={idx} className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1">
                        ...
                        {edu.field_of_study && (
                          <p className="text-slate-500">Major: {edu.field_of_study}</p>
                        )}
                        {edu.gpa && (
                          <p className="text-brand-600 font-semibold text-[11px]">GPA / Score: {edu.gpa}</p>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            {selectedSection === 'experience' && (
              ...
              >
                {parsed.experience?.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No formal employment history identified in resume.</p>
                ) : (
                  <div className="space-y-4">
                        ...
                        {exp.skills_used && exp.skills_used.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            {exp.skills_used.map((s, sIdx) => (
                              <span
                                key={sIdx}
                                className="text-[10px] bg-brand-50 text-brand-700 border border-brand-200 px-2 py-0.5 rounded"
                              >
                                {s}
                              </span>
                            ))}
            ...
            {selectedSection === 'projects' && (
              ...
              >
                {parsed.projects?.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No project entries detected in resume.</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {parsed.projects.map((proj, idx) => (
                      <div key={idx} className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-2 flex flex-col justify-between">
                        <div>
                          <h4 className="font-bold text-slate-900 text-sm">{proj.name}</h4>
                          <p className="text-slate-600 text-xs mt-1 line-clamp-3">{proj.description}</p>
                        </div>
                        <div>
                          {proj.technologies && proj.technologies.length > 0 && (
                            <div className="flex flex-wrap gap-1 pt-2">
                              {proj.technologies.map((t, tIdx) => (
                                <span
                                  key={tIdx}
                                  className="text-[10px] bg-slate-200 text-slate-700 px-2 py-0.5 rounded font-mono"
                                >
                                  {t}
                                </span>
                              ))}
                          ...
                          {proj.url && (
                            <a
                              ...
                              className="text-[11px] text-brand-600 hover:text-brand-700 font-semibold inline-block mt-2"
                            >
                              View Project Link →
                            </a>
                          )}
            ...
            {selectedSection === 'certifications' && (
              ...
              >
                {parsed.certifications?.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No professional certifications found in document.</p>
                ) : (
                  <div className="space-y-3">
