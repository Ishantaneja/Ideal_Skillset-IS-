    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center">
            <Terminal className="w-6 h-6 text-brand-600 mr-2" /> Practical Job Simulations
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Personalized coding challenges and business scenarios tailored for <strong className="text-slate-800">{data.target_role}</strong> candidates.
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200 flex items-center">
      ...
      <div className="grid grid-cols-1 gap-5">
        {data.challenges.map((c, idx) => (
          <Card key={c.id || idx} className="hover:border-brand-300 transition-all">
