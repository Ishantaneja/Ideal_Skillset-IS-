    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <Card className="bg-white border-slate-200 shadow-sm">
        <form onSubmit={handleRunAnalysis} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          ...
          <div className="md:col-span-2">
            ...
            >
              {analyzing ? <>Analyzing...</> : <><Layers className="w-4 h-4 mr-1.5" /> Analyze Gaps</>}
            </Button>
          </div>
        </form>
      </Card>
      ...
      {activeReport ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <Card title="Readiness Impact Summary" subtitle="Estimated matching score improvement">
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <span className="text-[10px] font-bold uppercase text-slate-400">Current Match</span>
                    <div className="text-2xl font-extrabold text-slate-800 mt-0.5">
                      {Math.round(activeReport.overall_gap_summary.current_ats_score)}%
                    </div>
                  </div>
                  <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                    <span className="text-[10px] font-bold uppercase text-emerald-600">Potential Match</span>
                    <div className="text-2xl font-extrabold text-emerald-700 mt-0.5">
                      {Math.round(activeReport.overall_gap_summary.potential_ats_score)}%
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-xs pt-2 border-t border-slate-100">
                  <div className="flex justify-between py-1">
                    <span className="text-slate-600 flex items-center font-medium">
                      <Flame className="w-3.5 h-3.5 mr-1.5 text-rose-500" /> Critical Skill Gaps
                    </span>
                    <span className="font-extrabold text-rose-700 bg-rose-50 px-2 py-0.5 rounded">
                      {activeReport.overall_gap_summary.critical}
                    </span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-600 flex items-center font-medium">
                      <AlertTriangle className="w-3.5 h-3.5 mr-1.5 text-amber-500" /> High Priority Gaps
                    </span>
                    <span className="font-extrabold text-amber-700 bg-amber-50 px-2 py-0.5 rounded">
                      {activeReport.overall_gap_summary.high}
                    </span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-600 flex items-center font-medium">
                      <CheckCircle2 className="w-3.5 h-3.5 mr-1.5 text-emerald-500" /> Verified Strengths
                    </span>
                    <span className="font-extrabold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded">
                      {activeReport.overall_gap_summary.matched_skills_count}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {activeReport.recommended_learning_order.length === 0 ? (
                ...
                <div className="space-y-2">
                  {activeReport.recommended_learning_order.map((skillName, idx) => (
                    <div
                      key={idx}
                      className="p-2.5 bg-white rounded-xl border border-slate-200 text-xs flex items-center justify-between hover:border-brand-300 transition-colors"
                    >
                      <div className="flex items-center space-x-2">
                        <span className="w-5 h-5 rounded-full bg-brand-100 text-brand-700 font-extrabold text-[10px] flex items-center justify-center">
                          {idx + 1}
                        </span>
                  ...
                  {history.map((item) => (
                    <div
                      ...
                      className={`p-3 rounded-xl border text-xs flex items-center justify-between ${
                        item.id === activeReport.id ? 'bg-brand-50/60 border-brand-300 shadow-2xs' : 'bg-white border-slate-200 hover:bg-slate-50'
                      }`}
                    >
                      <div
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Target Role Evaluation</span>
              ...
              <div className="bg-brand-50 border border-brand-200 rounded-xl p-3 flex items-center space-x-3 shrink-0">
                <Sparkles className="w-5 h-5 text-brand-600" />
                <div className="text-xs">
                  <p className="font-bold text-brand-900">Interactive Simulation</p>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg font-semibold transition-colors ${
                    filterStatus === 'gaps' ? 'bg-brand-600 text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg font-semibold transition-colors ${
                    filterStatus === 'mastered' ? 'bg-brand-600 text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg font-semibold transition-colors ${
                    filterStatus === 'all' ? 'bg-brand-600 text-white shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100'
                  }`}
                >
                  ...
                  return (
                    <Card
                      ...
                      className={`transition-all hover:border-brand-300 ${
                        skill.priority_score >= 8.5 ? 'border-l-4 border-l-rose-500' : skill.priority_score >= 7.0 ? 'border-l-4 border-l-amber-500' : 'border-l-4 border-l-slate-300'
                      }`}
                    >
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div className="flex items-center space-x-2.5">
                            <span className="w-6 h-6 rounded-lg bg-slate-900 text-white font-extrabold text-xs flex items-center justify-center shrink-0">
                              #{idx + 1}
                            </span>
                            <div>
                              <h3 className="text-base font-bold text-slate-900 flex items-center">
                                {skill.skill}
                                <span className="ml-2 text-[10px] text-slate-500 font-normal bg-slate-100 px-2 py-0.5 rounded">
                                  {skill.category}
                                </span>
                              </h3>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-slate-600 pt-1">
                          ...
                          <div className="flex items-center space-x-1.5">
                            ...
                            <span>Learning Budget: <strong className="text-slate-900">~{skill.estimated_learning_hours} hrs</strong></span>
                          </div>
                          <div className="flex items-center space-x-1.5">
                            <Briefcase className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                            <span>Role Density: <strong className="text-slate-900">{Math.round(skill.job_frequency * 100)}% of duties</strong></span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100">
                          ...
                          {skill.gap > 0 && (
                            <div className="flex items-center space-x-1.5 text-xs">
                              <span className="text-[11px] font-bold text-slate-500 flex items-center">
                                <Sparkles className="w-3 h-3 mr-1 text-brand-500" /> Simulate Target:
                              </span>
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            className="text-xs text-brand-600 font-bold ml-auto"
        ...
        <Card className="text-center py-16 px-6 border-dashed border-2 border-slate-200">
          <div className="max-w-md mx-auto space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-800">No skill gap analysis selected</h3>
      ...
      {selectedSkillDetail && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto shadow-xl border border-slate-200">
            ...
            <div className="grid grid-cols-3 gap-3 text-center text-xs">
              ...
              <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                <span className="text-[10px] uppercase font-bold text-emerald-600">ATS Match Gain</span>
              ...
              <div className="p-3 bg-brand-50 rounded-xl border border-brand-200">
                <span className="text-[10px] uppercase font-bold text-brand-600">Learning Effort</span>
                <div className="text-lg font-extrabold text-brand-700 mt-0.5">
            ...
            <div className="space-y-1.5 text-xs">
              <h4 className="font-bold text-slate-900 flex items-center">
                <ShieldCheck className="w-4 h-4 mr-1.5 text-brand-600" /> Actionable Learning Recommendation
              </h4>
              <p className="text-slate-600 bg-brand-50/40 p-3 rounded-xl border border-brand-200 leading-relaxed">
            ...
            {selectedSkillDetail.recommended_topics?.length > 0 && (
              <div className="space-y-2 text-xs">
                <h4 className="font-bold text-slate-900 flex items-center">
                  <BookOpen className="w-4 h-4 mr-1.5 text-slate-600" /> Recommended Practice Curriculum
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {selectedSkillDetail.recommended_topics.map((topic, i) => (
                    <div key={i} className="p-2 bg-slate-50 rounded-lg border border-slate-200 text-slate-700 flex items-center">
                      <span className="w-4 h-4 rounded-full bg-brand-100 text-brand-700 text-[10px] font-bold flex items-center justify-center mr-2 shrink-0">
                        {i + 1}
                      </span>
                      {topic}
                    </div>
            ...
            <div className="space-y-3 text-xs">
              <h4 className="font-bold text-slate-900">Verified Evidence Comparison</h4>
              <div className="space-y-2">
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                  <span className="font-bold text-slate-700">Resume Evidence (Current Level: {selectedSkillDetail.current_level_label}):</span>
                  {selectedSkillDetail.evidence?.resume?.length > 0 ? (
                    selectedSkillDetail.evidence.resume.map((r, i) => (
                      <p key={i} className="text-slate-600 italic text-[11px]">"{r}"</p>
                    ))
                  ) : (
                    <p className="text-slate-400 italic text-[11px]">No demonstrated evidence found in uploaded resume.</p>
                  )}
                </div>

                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                  <span className="font-bold text-slate-700">Job Description Requirement (Target: {selectedSkillDetail.required_level_label}):</span>
                  {selectedSkillDetail.evidence?.job?.length > 0 ? (
                    selectedSkillDetail.evidence.job.map((j, i) => (
                      <p key={i} className="text-slate-600 italic text-[11px]">"{j}"</p>
                    ))
                  ) : (
                    <p className="text-slate-400 italic text-[11px]">Listed as mandatory role requirement.</p>
                  )}
                </div>
              </div>
            </div>
